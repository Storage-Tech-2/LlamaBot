import { DefaultEndianDataView } from "./default_endian_data_view.js";
...
export class DataViewWriter {
  ...
  string(s: string) {
    ...
    const written = encodeUtf8Into(s, this.data.getView(), this.i + 2);
  ...
  array(array: DataView, width: number) {
    ...
    new Uint8Array(this.data.getView().buffer, this.i, array.byteLength)
  ...
  private assertCapacity(newBytes: number) {
    if (this.i + newBytes > this.data.getView().byteLength) {
      let newSize = this.data.getView().byteLength * 2;
      while (this.i + newBytes > newSize) {
        newSize *= 2;
      }
      const newData = new DefaultEndianDataView(this.littleEndian, new ArrayBuffer(newSize));
      new Uint8Array(newData.getView().buffer).set(new Uint8Array(this.data.getView().buffer));
      this.data = newData;
    }
  }
}
...
export class DataViewReader {
  ...
  string(): string {
    ...
    this.i += 2;
    const string = decodeUtf8(this.data.getView(), this.i, length);
    this.i += length;
    return string;
  }
  ...
  array(width: number): DataView {
    ...
    this.i += result.getView().byteLength;

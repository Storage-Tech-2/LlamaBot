    ...
    async execute(guildHolder: GuildHolder, interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        switch (subcommand) {
            ...
            case 'reject': {
                ...
                await submission.statusUpdated();
                await interaction.reply({
                    content: `<@${interaction.user.id}> has retracted this submission. It is no longer archived and cannot be archived in the future without a new post. Reason: ${reason || 'No reason provided'}`,
                });
                break;
            }

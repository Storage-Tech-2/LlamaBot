...
def embed(req: EmbedRequest, request: Request):
    with _MODEL_LOCK:
        ...
        try:
            if req.model_type == "document":
                embeddings = model.encode(req.texts, truncate_dim=256, normalize_embeddings=True)
            else:
                embeddings = model.encode(req.texts, truncate_dim=256, normalize_embeddings=True, prompt_name="query")
